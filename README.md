...
Replicated [Akka Persistence](http://doc.akka.io/docs/akka/2.4.5/scala/persistence.html) journal and snapshot store backed by [Couchbase](http://www.couchbase.com/).
...
To include the latest release of the Couchbase plugins into your `sbt` project, add the following lines to your `build.sbt` file:
    ...
    libraryDependencies += "com.productfoundry" %% "akka-persistence-couchbase" % "0.3.4"

This version of `akka-persistence-couchbase` depends on Akka 2.4.5 and Scala 2.11.8.
...
- All operations required by the Akka Persistence [journal plugin API](http://doc.akka.io/docs/akka/2.4.5/scala/persistence.html#journal-plugin-api) are fully supported.
...
- Implements the Akka Persistence [snapshot store plugin API](http://doc.akka.io/docs/akka/2.4.5/scala/persistence.html#snapshot-store-plugin-api).
